...
open class KvmLocalVars(
    ...
    fun setSlot(index: UInt, slot: KvmSlot) {
        slots[index.toInt()].num = slot.num
        slots[index.toInt()].ref = slot.ref
    }

    fun getSlot(index: UInt): KvmSlot {
        return KvmSlot().apply {
            this.ref = slots[index.toInt()].ref
            this.num = slots[index.toInt()].num
        }
    }
}
